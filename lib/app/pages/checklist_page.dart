...
class _ChecklistPageState extends State<ChecklistPage> {
  ...
  void initState() {
    ...
    context.read<CouchbaseService>().init().then(
        (value) async{
          final data = await context.read<CouchbaseService>().fetch(collectionName: 'checklist');
          print(data.toString());
        }
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(onPressed: () async{
        context.read<CouchbaseService>().add(data: {'id': 1 ,'name': 'Item', 'checked': true}, collectionName: 'checklist');
